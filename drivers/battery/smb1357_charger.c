...
static void smb1357_hw_init(struct i2c_client * client)
{
	...
	/* DCIN ADAPTER : 5V to 9V */
	smb1357_dcin_adapter(client, DCIN_5VTO9V);
	...
	/* Disable irq */
	smb1357_enable_irq(client, IRQ_CFG_REG, 0xFF, 0x04);
	smb1357_enable_irq(client, IRQ2_CFG_REG, 0xFF, 0x04);
	smb1357_enable_irq(client, IRQ3_CFG_REG, 0xFF, 0x01);
...
#if 0
	/* charging current */
	smb1357_set_charging_current(client,0);

	/* input current */
	smb1357_set_input_current_limit(client, 0);

	/* set float voltage */
	smb1357_set_vfloat(client, charger->pdata->chg_float_voltage);

	/* current termination level */
	data = 0xc8;
	smb1357_charger_i2c_write(client, CFG_3_REG, &data);
#endif
...
static int smb1357_get_charging_status(struct i2c_client *client)
{
	...
	smb1357_charger_i2c_read(client, STATUS_2_REG, &status_2);
	smb1357_charger_i2c_read(client, STATUS_4_REG, &status_4);
	smb1357_charger_i2c_read(client, STATUS_5_REG, &status_5);
	smb1357_charger_i2c_read(client, STATUS_7_REG, &status_7);
	smb1357_charger_i2c_read(client, STATUS_8_REG, &status_8);

	dev_info(&client->dev,"chg status:s1[0x%2x]s2[0x%2x],s4[0x%2x]\n",
		status_1, status_2, status_4);
	dev_info(&client->dev,"chg status:s5[0x%2x]s7[0x%2x],s8[0x%2x]\n",
		status_5, status_7, status_8);

	/* At least one charge cycle terminated,
	 * Charge current < Termination Current
	 */
	if (status_4 & BATT_DONE_STATUS) {
...
static int smb1357_get_charging_health(struct i2c_client *client)
{
	...
	dev_info(&client->dev,"health:S4[0x%2x]S7[0x%2x]S8[0x%2x]",
		status_4, status_7, status_8);
				...
				struct i2c_client *client)
{
	...
	*/
	...
	//TODO	smb1357_write_enable(charger->client, false);
...
static bool smb1357_is_pogo_event(struct i2c_client *client)
{
	...
	if (((data_f & IRQ_F_POWER_OK_BIT) && ((status & (DCIN_9V|DCIN_LV)) != 0)
...
static void smb1357_detbat_work(struct work_struct *work)
{
	...
	if ((smb1357_check_vf_status(client) == false) ||
		(smb1357_get_batt_status(client) == false)) {
		smb1357_enter_suspend(client);
	}
				...
				const char *buf, size_t count)
{
	...
	dev_info(&charger->client->dev,
		...
		smb1357data->sysfs_mode,smb1357data->sysfs_curin,
		smb1357data->sysfs_curout);
				...
				struct device_attribute *attr, char *buf)
{
	...
	case CUR_SET:
		...
		case 2:
			...
			smb1357data->sysfs_mode,
			smb1357data->sysfs_curin, smb1357data->sysfs_curout);
			break;
		case USE_SYSFS_CUR:
				...
				const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d\n", &temp) == 1) {
		...
		smb1357data->chg_high_temp_recovery = temp - 50;
		ret = count;
	}
...
static void smb1357_charger_pogo_det_work(struct work_struct *work)
{
	...
	if (gpio == false) {
		...
		// TODO Send Insert/remove Event.
		dev_dbg(&client->dev," %s, Send POGO Event, %dv \n",
			__func__,smb1357data->pogo_status);
...
static void smb1357_charger_init_work(struct work_struct *work)
{
	...
	dev_dbg(&client->dev,"============================================\n");
	schedule_delayed_work(&smb1357data->init_work, msecs_to_jiffies(10000));
	return;
}
#endif
						...
						const struct i2c_device_id *id)
{
	...
	if (smb1357data->detbat_irq){
		...
		ret = request_threaded_irq(smb1357data->detbat_irq,
				NULL, smb1357_detbat_irq_thread,0,
				"detbat-irq", charger);
		if (ret) {
			dev_err(&client->dev,
		...
		ret = enable_irq_wake(smb1357data->detbat_irq);
		if (ret < 0)
			dev_err(&client->dev,
				"%s: Failed to Enable Wakeup Source DETBAT(%d)\n",
				__func__, ret);
	}
