...
#define TX_NUM		19
#define RX_NUM		12
#define NODE_NUM	240 /* 19x12 + key node */
...
extern eMFSRet_t MFS_ISC_update(struct i2c_client *client);
#endif

#define GPIO_TOUCH_INT  		1
...
#define TSP_DEVICE_NAME2 "mms134s_ts"
...
static const uint16_t SCR_ABS_UPPER_SPEC[MAX_RX_][MAX_TX_] = {
	{2062,1750,2049,1878,2055,1918,2057,1918,2065,1934,2070,1933,2068,1933,2030,1887,2029,1915,2020},
	{1851,1707,1801,1834,1874,1872,1875,1867,1882,1877,1890,1867,1863,1868,1848,1851,1878,1879,1883},
	{1853,1713,1794,1835,1872,1874,1873,1869,1879,1879,1884,1869,1852,1872,1847,1859,1882,1885,1889},
	{1859,1719,1794,1835,1871,1873,1871,1867,1877,1877,1882,1867,1849,1871,1845,1857,1880,1884,1889},
	{1862,1720,1796,1838,1870,1870,1869,1864,1874,1875,1880,1865,1847,1869,1841,1853,1877,1881,1888},
	{1868,1727,1798,1840,1873,1880,1871,1877,1878,1891,1886,1883,1856,1888,1837,1866,1870,1893,1874},
	{1877,1738,1797,1832,1873,1878,1884,1873,1892,1884,1900,1875,1870,1880,1859,1854,1892,1880,1887},
	{1873,1734,1795,1830,1870,1876,1873,1870,1878,1880,1884,1870,1852,1873,1848,1861,1881,1888,1893},
	{1872,1731,1794,1830,1869,1878,1874,1872,1880,1881,1885,1871,1853,1874,1850,1863,1884,1889,1895},
	{1863,1725,1789,1824,1864,1876,1872,1870,1878,1880,1883,1869,1851,1872,1848,1862,1882,1886,1891},
	{1861,1726,1787,1827,1862,1877,1874,1873,1879,1883,1883,1874,1850,1876,1844,1863,1879,1885,1886},
	{1911,2046,1837,2046,1910,2051,1919,2050,1930,2054,1943,2054,1928,2059,1889,2023,1916,2021,1921},
};

static const uint16_t SCR_ABS_LOWER_SPEC[MAX_RX_][MAX_TX_] = {
	{1111,942,1103,1011,1107,1033,1108,1033,1112,1042,1114,1041,1113,1041,1093,1016,1092,1031,1087},
	{997,919,970,988,1009,1008,1010,1005,1013,1011,1017,1005,1003,1006,995,997,1011,1012,1014},
	{998,922,966,988,1008,1009,1009,1006,1012,1012,1014,1006,997,1008,995,1001,1013,1015,1017},
	{1001,926,966,988,1008,1009,1008,1005,1011,1011,1013,1005,996,1007,993,1000,1012,1015,1017},
	{1003,926,967,990,1007,1007,1006,1004,1009,1010,1012,1004,995,1006,992,998,1011,1013,1017},
	{1006,930,968,991,1008,1012,1008,1011,1011,1018,1015,1014,999,1017,989,1005,1007,1019,1009},
	{1011,936,968,987,1009,1011,1014,1009,1019,1014,1023,1009,1007,1012,1001,998,1019,1012,1016},
	{1009,934,967,985,1007,1010,1008,1007,1011,1012,1015,1007,997,1008,995,1002,1013,1017,1020},
	{1008,932,966,985,1006,1011,1009,1008,1012,1013,1015,1007,998,1009,996,1003,1014,1017,1020},
	{1003,929,963,982,1004,1010,1008,1007,1011,1012,1014,1006,997,1008,995,1002,1013,1016,1018},
	{1002,929,962,984,1003,1011,1009,1008,1012,1014,1014,1009,996,1010,993,1003,1012,1015,1016},
	{1029,1102,989,1102,1029,1104,1033,1104,1039,1106,1046,1106,1038,1109,1017,1089,1032,1088,1035},
};
...
#define MELFAS_MULI_TOUCH_CHECK		// When press or release, display one time.
#ifdef MELFAS_MULI_TOUCH_CHECK
static int muti_touch_check[MELFAS_MAX_TOUCH];
#endif
...
#define TSP_PWR_GPIO 60
void ts_power_enable(int en)
{
	printk(KERN_ERR "%s: (%d)\n", __func__, en);
	if (en) {
		gpio_direction_output(TSP_PWR_GPIO, 1);
	} else {
		gpio_direction_output(TSP_PWR_GPIO, 0);
	}
...
#if DEBUG_PRINT2
static char get_data[4096];

static int get_intensity1(struct melfas_ts_data *ts){
	...
	memset(get_data,0,4096);
	sprintf(get_data,"start intensity\n");
	for(r = 0 ; r < rx_num ; r++)
	{
		tmp = 0;
	...
	if (key_num)
	{
		...
		while (gpio_get_value(ts->irq_gpio)){
		}

		sz = i2c_smbus_read_byte_data(ts->client, 0xAE);
...
static ssize_t mms_intensity(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	int ret;
	dev_info(&ts->client->dev, "Intensity Test\n");
...
#if 0  // old
	if (!ts->noise_mode){
		printk(KERN_INFO "[TSP] not noise mode, TA:%d \n",ts->ta_status );
		return;
	}

	if (ts->ta_status) {
		printk(KERN_INFO "[TSP] TA connect!!\n");
		setLowLevelData[0] = 0x30;
		setLowLevelData[1] = 0x01;
		ret = melfas_i2c_write(ts->client, setLowLevelData, 2);
		}
	else {
		printk(KERN_INFO "[TSP] TA disconnect!!\n");
		setLowLevelData[0] = 0x30;
...
static int ta_state = 99;
void charger_enable(int enable)
{
	//static int ta_state = 99;
	printk(KERN_INFO "[TSP] %s, enable:%d, old:%d\n", __func__, enable, ta_state);

	if (ts == NULL)
		return;
	if(ta_state == 99){			 		// first
		ts->ta_status = !(!(enable));
		ta_state = ts->ta_status;
	...
	if (enable == 0){
		ts->ta_status = 0;
		if(ta_state==0) return;			// duplicate
		ta_state = 0;
...
static void melfas_ts_get_data(struct work_struct *work)
{
	...
	for (i = 0; i < MELFAS_MAX_TOUCH; i++) {
		...
		if (g_Mtouch_info[i].strength) {
			...
			#ifdef MELFAS_MULI_TOUCH_CHECK
			printk(KERN_ERR "[TSP] ID:%d, R:%d, (%d,%d), z:%d w:%d\n",
			i, (g_Mtouch_info[i].strength > 0),	g_Mtouch_info[i].posX, g_Mtouch_info[i].posY, g_Mtouch_info[i].strength, g_Mtouch_info[i].width);
			muti_touch_check[i] = 0;
			#endif
		}
#ifndef MELFAS_MULI_TOUCH_CHECK
...
static u16 key_CmDelta_data[12] = { 0, }; /* inspection */
static u16 CmABS_data[NODE_NUM] = { 0, }; /* reference */
static u16 key_CmABS_data[12] = { 0, }; /* reference */
...
//	gpio = msm_irq_to_gpio(ts->irq);
	...
	if (cmd == MMS_VSC_CMD_CM_DELTA || cmd == MMS_VSC_CMD_CM_ABS) {
		...
		while (gpio_get_value(GPIO_TSP_INT))
			udelay(100);
	}
	max_value = 0;
	min_value = 0;
...
static int check_rx_tx_num(void *device_data)
{
	...
	if(ts->cmd_param[0]==19){		//touchkey node
		if(ts->cmd_param[1]==3)
			node=228;
		if(ts->cmd_param[1]==8)
			node=229;
...
static void fw_update(void *device_data)
{
	...
	for (i = 0; i < DOWNLOAD_RETRY_CNT; i++) {
		...
		if (ret != 0) {
			printk(KERN_ERR "SET Download Fail in factory mode-error code [%d]\n", ret);
			...
			case MRET_CHECK_COMPATIBILITY_ERROR:
				dev_err(&client->dev,"fw version update does not need - old module\n");
...
static void get_fw_ver_bin(void *device_data)
{
	...
	char buff[16] = {0};
	set_default_result(ts);
	snprintf(buff, sizeof(buff), "ME0003%02x", NEW_FIRMWARE_VERSION);
	set_cmd_result(ts, buff, strnlen(buff, sizeof(buff)));
	ts->cmd_state = 2;
	dev_info(&ts->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
static void get_fw_ver_ic(void *device_data)
{
	...
	ret = melfas_i2c_read(ts->client, 0xE3, 1, &read_data_buf);
	dev_info(&ts->client->dev, "%s: ret:%d, data:%d,%d \n", __func__, ret, TSP_PanelVersion, read_data_buf);
	if (ret < 0)
		ts->fw_ic_ver = TSP_PanelVersion;
	else
		ts->fw_ic_ver = read_data_buf;
#else	// only one read in booting.

	ts->fw_ic_ver = TSP_PanelVersion;
#endif
	set_default_result(ts);
	ver = ts->fw_ic_ver;
	snprintf(buff, sizeof(buff), "ME0003%02x", ver);
	set_cmd_result(ts, buff, strnlen(buff, sizeof(buff)));
	ts->cmd_state = 2;
	dev_info(&ts->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
//	u16 read_data_buf1[50] = {0,};
	...
	/* event type check */
		...
		while (retry) {
			...
			if (ret < 0){
				printk(KERN_ERR " i2c error - r:%d ", ret);
				break;
			}
			...
			if(retry++ == 10){
				printk(KERN_ERR " retry 10, break - r:%d ", retry);
				break;
			}
		}
	}
...
*/
	...
	for (key=0; key< 2; key++) {
			setLowLevelData[0] = 0xA0;
			setLowLevelData[1] = 0x4B;
			setLowLevelData[2] = key;
			setLowLevelData[3] = 0;
			ret = melfas_i2c_write(ts->client, setLowLevelData, 4);
			while (gpio_get_value(GPIO_TOUCH_INT))
				;
			...
			ts->raw[(exciting_ch * sensing_ch)+key]=key_CmABS_data[key];
			printk( "[key%d] %4d ",key, key_CmABS_data[key]);
			if((955<key_CmABS_data[key])&&(key_CmABS_data[key]<1782))
				status = 0; /* pass */
			else
				status = 1; /* fail */
	}
...
//	u16 read_data_buf1[50] = {0,};
	...
	for (key=0; key< 2; key++) {
			...
			ret = melfas_i2c_read(ts->client, 0xAF,
				size, read_data_buf);
			key_CmDelta_data[key]
				= (read_data_buf[0] |  read_data_buf[1] << 8);

			ts->inspection[(exciting_ch * sensing_ch)+key]=key_CmDelta_data[key];
			printk( "[key%d] %4d ",key, key_CmDelta_data[key]);
			if((190<key_CmDelta_data[key])&&(key_CmDelta_data[key]<323))
				status = 0; /* pass */
			else
				status = 1; /* fail */
	}
...
printk("[TSP] %s : %d \n",__func__,__LINE__);
	...
	error = gpio_request(TSP_PWR_GPIO, "tsp_en_gpio");
	if (error) {
		printk("[TSP] unable to request gpio [%d]\n", TSP_PWR_GPIO);
	}
	ts_power_enable(0);
	msleep(60);
	ts_power_enable(1);
	...
	if (is_lcd_attached() != 0) {
		...
		printk("[TSP] %s : is_lcd_attached()=0 \n",__func__);
	}
#endif
#endif
	if(ta_state == 99){		// init value, ta_status = 0;
		ts->ta_status = 0;
	}

	ts->irq_gpio = of_get_named_gpio_flags(np, "melfas,irq-gpio",
				0, &ts->irq_gpio_flags);


	if (gpio_is_valid(ts->irq_gpio)) {
		/* configure touchscreen irq gpio */
		error = gpio_request(ts->irq_gpio, "ms_irq_gpio");
		if (error) {
			dev_err(&client->dev, "unable to request gpio [%d]\n",
						ts->irq_gpio);
		}
		error = gpio_direction_input(ts->irq_gpio);
		if (error) {
			dev_err(&client->dev,
				"unable to set direction for gpio [%d]\n",
				ts->irq_gpio);
		}
		ts->irq = client->irq = gpio_to_irq(ts->irq_gpio);
	} else {
		dev_err(&client->dev, "irq gpio not provided\n");
	}
#if 0
 		ts->gpio_ldo_en_num	= of_get_named_gpio_flags(np, "vdd_en-gpio",	0, &ts->gpio_ldo_en_num);

		if (gpio_is_valid(ts->gpio_ldo_en_num)) {
			/* configure touchscreen irq gpio */
			error = gpio_request(ts->gpio_ldo_en_num, "melfas_ldo_gpio");
			if (error) {
				dev_err(&client->dev, "unable to request gpio [%d]\n",
							ts->int_gpio_num);
			}
			error = gpio_direction_output(ts->gpio_ldo_en_num, 1);
			if (error) {
				dev_err(&client->dev,
					"unable to set direction for gpio [%d]\n",
					ts->gpio_ldo_en_num);
	...
	ts->input_dev->name = TSP_DEVICE_NAME;
	...
	ts->input_dev->open = melfas_ts_open;
	ts->input_dev->close = melfas_ts_close;
	...
	if (IC_type == 0x0F)
		...
		#ifdef CONFIG_MACH_KYLE_I
		ts->config_fw_version = "CS02_1002_GF1";
		#endif
	}
	#endif
...
//fac_dev_ts
	...
	ret = sysfs_create_group(&sec_touchscreen_dev->kobj,
			       &sec_touch_factory_attr_group);
	if (ret)
		dev_err(&client->dev, "Failed to create sysfs group\n");
#endif
#if defined(USE_TEST_RAW_TH_DATA_MODE)
...
#if DEBUG_PRINT2
	if (sysfs_create_link(NULL, &client->dev.kobj,TSP_DEVICE_NAME)) {
...
#if DEBUG_PRINT2
	sysfs_remove_link(NULL, TSP_DEVICE_NAME);
