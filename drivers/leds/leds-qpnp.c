...
static int qpnp_flash_set(struct qpnp_led_data *led)
{
	...
	if (val > 0) {
		if (led->flash_cfg->torch_enable) {
			...
			rc = qpnp_led_masked_write(led, FLASH_MAX_CURR(led->base),FLASH_CURRENT_MASK, TORCH_MAX_LEVEL);
			if (rc) {
				dev_err(&led->spmi_dev->dev,
			...
			rc = qpnp_led_masked_write(led, FLASH_LED_TMR_CTRL(led->base), FLASH_TMR_MASK, FLASH_TMR_WATCHDOG);
			if (rc) {
				dev_err(&led->spmi_dev->dev,
			...
			rc = qpnp_led_masked_write(led, FLASH_WATCHDOG_TMR(led->base), FLASH_WATCHDOG_MASK, led->flash_cfg->duration);
			if (rc) {
				dev_err(&led->spmi_dev->dev,
			...
			rc = qpnp_led_masked_write(led, FLASH_LED_TMR_CTRL(led->base), FLASH_TMR_MASK, FLASH_TMR_SAFETY);
