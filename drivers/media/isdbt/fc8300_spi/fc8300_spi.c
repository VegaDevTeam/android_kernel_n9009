...
static int __devinit fc8300_spi_probe(struct spi_device *spi)
{
	...
	spi->max_speed_hz = 48000000; //52000000
	spi->bits_per_word = 8;
	spi->mode =  SPI_MODE_0;
	...
	if(spi == NULL)
	pr_err("fc8300_spi_probe spi == NULL 2\n");
	
	fc8300_spi = spi;
	if(fc8300_spi == NULL)
	pr_err("fc8300_spi_probe spi == NULL 3\n");
	
	print_log(0, "fc8300_spi_probe end spi handle : 0x%x\n", fc8300_spi);
		...
		u16 addr, u8 command, u8 *data, u16 length)
{
	...
	for (i = 0 ; i < length ; i++)
	{
		if( (4+i) < 10)			//Fix PLM P140206-02359
		{
			tx_data[4+i] = data[i];
		}
		else		
