	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
		...
		if (enable) {
			rc = gpio_tlmm_config(GPIO_CFG(ctrl_pdata->disp_en_gpio, 0,
						...
						GPIO_CFG_ENABLE);
			if (rc)
				pr_err("enabling disp_en_gpio[%d] failed, rc=%d\n", ctrl_pdata->disp_en_gpio, rc);
			...
			rc = gpio_tlmm_config(GPIO_CFG(ctrl_pdata->disp_en_gpio, 0,
						...
						GPIO_CFG_ENABLE);
			if (rc)
				pr_err("disabling disp_en_gpio failed, rc=%d\n",rc);
...
void ktd253_set_brightness(int level, struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if(level == 0){
		gpio_set_value((ctrl_pdata->bklt_ctrl), 0);
	...
	if(tune_level != lcd_brightness)
	{
		if (!tune_level) {
			...
			if (unlikely(lcd_brightness < 0)) {
				int val = gpio_get_value(ctrl_pdata->bklt_ctrl);
				if (val) {
					lcd_brightness = 0;
				gpio_set_value(ctrl_pdata->bklt_ctrl, 0);
				mdelay(3);
			...
			if (!lcd_brightness) {
				gpio_set_value(ctrl_pdata->bklt_ctrl, 1);
				udelay(3);
				lcd_brightness = 1;
			}
	...
	bool cmd_cfg_cont_splash)
{
	...
	gpio_tlmm_config(GPIO_CFG(ctrl_pdata->bklt_ctrl, 0,
						GPIO_CFG_OUTPUT,GPIO_CFG_PULL_DOWN,GPIO_CFG_2MA),
						GPIO_CFG_ENABLE);

	gpio_tlmm_config(GPIO_CFG(23,  0, GPIO_CFG_INPUT,
					GPIO_CFG_PULL_UP, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
	rc = gpio_direction_input(23);
	if (unlikely(rc < 0)) {
		pr_err("%s: failed to set gpio LCD CHECK as input (%d)\n",
			__func__, rc);
	}
