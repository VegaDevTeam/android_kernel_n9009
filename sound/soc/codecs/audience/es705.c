...
int es705_put_veq_block(int volume)
{
	...
	do {
		ret = es705->dev_read(es705, (char *)&resp,
				ES705_READ_VE_WIDTH);
		dev_dbg(es705->dev, "%s: wdb_cmd : send data block length ret = %d response = 0x%08x\n",
				__func__, ret, resp);
		count++;
	} while (resp != cmd && count < 5);

	if (resp != cmd) {
		dev_err(es705->dev, "%s(): read the response failed 0x%08x\n",
				__func__, resp);
		goto EXIT;
	}
	...
	do {
		...
		ret = es705->dev_read(es705, (char *)&fin_resp,
				ES705_READ_VE_WIDTH);
		dev_dbg(es705->dev, "%s: wdb : send data block ret = %d response = 0x%08x\n",
			__func__, ret, fin_resp);
		count++;
	} while (fin_resp != 0x802f0000 && count < 5);
